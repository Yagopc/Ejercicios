...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnBuscar').addEventListener('click', abrirSelectorFecha);
    document.getElementById('btnCargarPorFecha').addEventListener('click', buscarPorFecha);
    ...
    // Cargar datos iniciales desde el archivo local
...
function abrirSelectorFecha() {
    ...
    fechaContainer.style.display = fechaContainer.style.display === 'none' ? 'block' : 'none';
}

// Función para buscar ejercicio por fecha
function buscarPorFecha() {
    const fechaSeleccionada = document.getElementById('selectorFecha').value;
    if (!fechaSeleccionada) {
        mostrarNotificacion('Por favor, selecciona una fecha');
        return;
    }
    
    // Formatear la fecha a YYYY-MM-DD para comparar
    const fechaBuscada = fechaSeleccionada;
    
    // Buscar en los datos
    const ejercicioEncontrado = datosEjercicios.find(ej => {
        const fechaEjercicio = ej.fecha.split('T')[0]; // Eliminar la parte de tiempo si existe
        return fechaEjercicio === fechaBuscada;
    });
    
    if (ejercicioEncontrado) {
        // Rellenar los campos con los datos encontrados
        document.getElementById('tipoDeporte').value = ejercicioEncontrado.tipo;
        document.getElementById('tiempo').value = ejercicioEncontrado.tiempo;
        document.getElementById('edad').value = ejercicioEncontrado.edad;
        document.getElementById('peso').value = ejercicioEncontrado.peso;
        document.getElementById('velocidad').value = ejercicioEncontrado.velocidad;
        
        // Actualizar resultados actuales
        resultadosActuales = {
            fecha: ejercicioEncontrado.fecha,
            tiempo: ejercicioEncontrado.tiempo,
            edad: ejercicioEncontrado.edad,
            peso: ejercicioEncontrado.peso,
            calorias: ejercicioEncontrado.calorias,
            distancia: ejercicioEncontrado.distancia,
            velocidad: ejercicioEncontrado.velocidad,
            tipo: ejercicioEncontrado.tipo
        };
        
        // Mostrar resultados
        mostrarResultados();
        mostrarNotificacion('Datos del ' + new Date(fechaBuscada).toLocaleDateString() + ' cargados');
    } else {
        mostrarNotificacion('No hay datos para el día seleccionado');
    }
}

// Resto de las funciones permanecen iguales (cargarDatosIniciales, cargarUltimaSesion, etc.)
// ... [El resto del código de script.js permanece exactamente igual]
function cargarDatosIniciales() {
    try {
        // Simulamos la carga de datos iniciales con el archivo proporcionado
        datosEjercicios = [
            {
                "fecha": "2025-06-11",
                "tiempo": 30,
...
function actualizarTotales() {
    ...
    datosEjercicios.forEach(ej => {
        ...
        if (!ultimaFecha || new Date(ej.fecha) > new Date(ultimaFecha)) {
            ultimaFecha = ej.fecha;
        }
    });
    ...
    if (ultimaFecha) {
        ...
        document.getElementById('ultima-fecha').textContent = fechaFormateada;
    } else {
        document.getElementById('ultima-fecha').textContent = 'N/A';
    }
